...
The reusable architecture problem was bigger than adding aliases. Ex line commands cross the parser, range handling, destination addresses, pure prompt-buffer transforms, unnamed register semantics, visible search highlights, transient Ex messages, macro replay, dot-repeat boundaries, docs, and tests. Later range-algebra work extended the same architecture: finite Ex address/range/destination parsing now lives in a pure `src/range.ts` kernel, while `src/ex.ts`, `src/buffer.ts`, and `src/modal/engine.ts` stay responsible for syntax dispatch, prompt-buffer operations, and editor side effects respectively.
...
6. **Keep side effects in `src/modal/engine.ts`.** `executeExCommand` is the chokepoint for applying parsed command results:
...
7. **Lock behavior at three levels.** Use parser tests for grammar, buffer tests for pure line transforms, and editor integration tests for modal side effects. For the range-algebra extension, validation covered `test/range.test.ts`, `test/ex.test.ts`, `test/buffer.test.ts`, and modal Ex regressions for offset/semicolon preview/apply, invalid range safety, destination `0`, register writes, search-highlight behavior, and dot-repeat preservation. Keep the durable validation checklist small: OpenSpec validation, Bun tests, typecheck, lint, and formatter.
...
This also prevents Vim-parity drift. A user can rely on the documented finite commands and finite range grammar, while future implementers can see that Ex register operands, `:global`, shell/file/window commands, arbitrary expressions, repeated offsets, and Vimscript evaluation remain intentionally out of scope.
...
- `openspec/changes/archive/2026-06-01-add-more-ex-commands/` — source OpenSpec change for the finite line-command implementation.
