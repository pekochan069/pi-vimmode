...
The Vim editor SHALL read extension settings from a `piVimMode` object without modifying Pi settings files.

#### Scenario: Namespaced settings loaded

- **WHEN** Pi starts a session with `piVimMode` configured in global or project settings
- **THEN** the extension reads only the supported `piVimMode.startMode` and `piVimMode.cursor` fields for Vim editor behavior
